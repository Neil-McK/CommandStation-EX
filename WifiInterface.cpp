...
wifiSerialState WifiInterface::setup2(const FSH* SSid, const FSH* password,
  ...
  if (strncmp_P(yourNetwork, (const char *)SSid, 13) == 0 ||
    ...
    if (strncmp_P(yourNetwork, (const char *)password, 13) == 0) {
      ...
      if (checkForOK(5000, F("+CIFSR:STAIP"), true, false))
        if (!checkForOK(1000, F("0.0.0.0"), true, false)) ipOK = true;
    }
  } else {
  ...
  if (!ipOK) {
    ...
    do {
      if (strncmp_P(yourNetwork, (const char *)password, 13) == 0) {
        // unconfigured
        StringFormatter::send(wifiStream,
                              F("AT+CWSAP%s=\"DCCEX_%s\",\"PASS_%s\",%d,4\r\n"),
                              oldCmd ? "" : "_CUR", macTail, macTail, channel);
      } else {
        // password configured by user
        StringFormatter::send(wifiStream,
                              F("AT+CWSAP%s=\"DCCEX_%s\",\"%S\",%d,4\r\n"),
  ...
  // Copy the IP address
  {
    ...
    LCD(4, F("%s"), ipString);  // There is not enough room on some LCDs to put
                                // a title to this
  }
...
// DCCEXParser Once the user has made whatever changes to the AT commands, a
// <+X> command can be used to force on the connectd flag so that the loop will
// start picking up wifi traffic. If the settings are corrupted <+RST> will
// clear this and then you must restart the arduino.
