...
void WiThrottle::parse(RingStream* stream, byte* cmdx) {
  ...
  if (initSent) {
    ...
    // Send turnout list if changed since last sent (will replace list on
    // client)
    if (turnoutListHash != Turnout::turnoutlistHash) {
      StringFormatter::send(stream, F("PTL"));
      for (Turnout* tt = Turnout::firstTurnout; tt != NULL;
           tt = tt->nextTurnout) {
        StringFormatter::send(stream, F("]\\[%d}|{%d}|{%c"), tt->data.id,
                              tt->data.id,
                              Turnout::isActive(tt->data.id) ? '4' : '2');
      }
      StringFormatter::send(stream, F("\n"));
      turnoutListHash =
          Turnout::turnoutlistHash;  // keep a copy of hash for later comparison
    }
  }
    ...
    switch (cmd[0]) {
      ...
      case 'P':
        if (cmd[1] == 'P' && cmd[2] == 'A') {  // PPA power mode
          DCCWaveform::mainTrack.setPowerMode(cmd[3] == '1' ? POWERMODE::ON
                                                            : POWERMODE::OFF);
          if (MotorDriver::commonFaultPin)  // commonFaultPin prevents
                                            // individual track handling
            DCCWaveform::progTrack.setPowerMode(cmd[3] == '1' ? POWERMODE::ON
                                                              : POWERMODE::OFF);
          StringFormatter::send(
              stream, F("PPA%x\n"),
              DCCWaveform::mainTrack.getPowerMode() == POWERMODE::ON);
          lastPowerState = (DCCWaveform::mainTrack.getPowerMode() ==
                            POWERMODE::ON);  // remember power state sent for
                                             // comparison later
      ...
      case 'H':  // send initial connection info after receiving "HU" message
        if (cmd[1] == 'U') {
          ...
          if (annotateLeftRight)
            StringFormatter::send(
                stream,
                F("PTT]\\[Turnouts}|{Turnout]\\[Left}|{2]\\[Right}|{4\n"));
          else
            StringFormatter::send(
                stream,
                F("PTT]\\[Turnouts}|{Turnout]\\[Closed}|{2]\\[Thrown}|{4\n"));
          StringFormatter::send(
              stream, F("PPA%x\n"),
              DCCWaveform::mainTrack.getPowerMode() == POWERMODE::ON);
          lastPowerState = (DCCWaveform::mainTrack.getPowerMode() ==
    ...
    case '+':  // add loco request
      ...
      for (int loco = 0; loco < MAX_MY_LOCO; loco++) {
        if (myLocos[loco].throttle == '\0') {
          ...
          StringFormatter::send(stream, F("M%c+%c%d<;>\n"), throttleChar,
                                cmd[3], locoid);  // tell client to add loco
          ...
          StringFormatter::send(stream, F("M%cA%c%d<;>V%d\n"), throttleChar,
                                cmd[3], locoid,
                                DCCToWiTSpeed(DCC::getThrottleSpeed(locoid)));
          StringFormatter::send(stream, F("M%cA%c%d<;>R%d\n"), throttleChar,
                                cmd[3], locoid,
                                DCC::getThrottleDirection(locoid));
  ...
  switch (aval[0]) {
    ...
    case 'q':
      if (aval[1] == 'V') {  // qV
        LOOPLOCOS(throttleChar, cab) {
          StringFormatter::send(
              stream, F("M%cA%c%d<;>V%d\n"), throttleChar,
              LorS(myLocos[loco].cab), myLocos[loco].cab,
              DCCToWiTSpeed(DCC::getThrottleSpeed(myLocos[loco].cab)));
        }
      } else if (aval[1] == 'R') {  // qR
        LOOPLOCOS(throttleChar, cab) {
          StringFormatter::send(stream, F("M%cA%c%d<;>R%d\n"), throttleChar,
    ...
    case 'R': {
      ...
      LOOPLOCOS(throttleChar, cab) {
        DCC::setThrottle(myLocos[loco].cab,
                         DCC::getThrottleSpeed(myLocos[loco].cab), forward);
        StringFormatter::send(stream, F("M%cA%c%d<;>R%d\n"), throttleChar,
                              LorS(myLocos[loco].cab), myLocos[loco].cab,
                              forward);
      }
    ...
    case 'X':
      ...
      LOOPLOCOS(throttleChar, cab) {
        DCC::setThrottle(myLocos[loco].cab, 1,
                         DCC::getThrottleDirection(myLocos[loco].cab));
        StringFormatter::send(stream, F("M%cA%c%d<;>V%d\n"), throttleChar,
                              LorS(myLocos[loco].cab), myLocos[loco].cab, -1);
      }
      break;
    ...
    case 'Q':  // Quit, set speed to 0
      LOOPLOCOS(throttleChar, cab) {
        DCC::setThrottle(myLocos[loco].cab, 0,
                         DCC::getThrottleDirection(myLocos[loco].cab));
        StringFormatter::send(stream, F("M%cA%c%d<;>V%d\n"), throttleChar,
                              LorS(myLocos[loco].cab), myLocos[loco].cab, 0);
      }
      break;
  }
}
...
char WiThrottle::LorS(int cab) { return (cab < 127) ? 'S' : 'L'; }
